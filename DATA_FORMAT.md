...
## Atlas rendering and compatibility

Проект остаётся v2: размер мира и `cellSize=5` не меняются. `detailModel: {version:1, salt:"worldsmith"}` задаёт версию и соль производной детализации. При загрузке старого v2 блок добавляется с этими значениями; миграция v1 создаёт тот же блок. Неизвестные версии отклоняются. В v0.6 художественная обработка обновлена для всех карт; география старых сохранений не перегенерируется. Макроиллюстрации сохраняются на всех масштабах до 200%, материалы не сохраняются в JSON.
...
**Тайловый атлас (необязательный экспорт):** `project.json` хранит исходный v2; `map/manifest.json` — renderModel (atlas-v6), размеры уровней, масштаб до 2×, DPR-профиль, SHA-256 проекта, версию деталей, ревизию, охват и завершённость; `map/overview.webp` — обзор; `map/tiles/<level>/<x>/<y>.webp` — тайлы 512×512 с прозрачным дополнением по краям. `vectors/labels.json`, `pois.json`, `roads.geojson`, `rivers.geojson`, `regions.geojson` содержат редактируемые данные. Границы регионов — MultiLineString по ячейкам; координаты всех GeoJSON — **пиксели мира, не долгота/широта**. Подписи и рамка не входят в тайлы. Дороги, реки и иллюстрации POI пока входят и в растр, и в структурированные данные. `coverage.partial:true` явно обозначает экспорт только выбранных границ. `complete:false` обозначает незавершённую запись. Подробнее: [SEMANTIC_ZOOM.md](SEMANTIC_ZOOM.md).
...
## v0.7 — план генерации мира

Формат остаётся **v2**. Новый необязательный `worldLayout` содержит `version:1`, `template` (river-crown / broken-crescent / eastern-wall), шесть `zones` с `id`, `character`, `position:{x,y}` и `radius:{x,y}`, а также короткие массивы `ridges` и `valleys` (`id`, `width`, `points`; для хребтов также `strength`). Все координаты — мировые пиксели. Валидатор проверяет версию, размеры, границы и допустимые характеры. План занимает несколько килобайт, не содержит списков производных деревьев или чанков.

Это происхождение генерации, а не повторно исполняемый рецепт при загрузке или правке кистью. У старых v1/v2 файлов поле остаётся отсутствующим; география и объекты не меняются. Источник истины после генерации — существующие массивы и структурированные пути. `regions[].character` описывает характер области. Новые дороги могут хранить `from`, `to`, `character`, `condition`; POI — `properties.storyRole`, `layoutZone`, `crossingRiver` или `route`. Старые объекты без этих метаданных поддерживаются.

Производные экспортные пакеты используют `renderModel:atlas-v7`, `cache.algorithm:3`; изменение формата сохранения для этого не требуется.
