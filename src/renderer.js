(function(W){
 ...
 function render(p,canvas,selectedId=null,options={}){
  ...
  if(['Height','Biome','Region'].includes(mode)){
   ...
   if(on('Labels')&&options.labels!==false)labels(c,p,selectedId,on);if((!only||only==='Frame')&&(options.frame??p.styling.frame))frame(c,p);c.restore();return;
  }
  ...
  const passes={Terrain(){W.surface?.details(c,p,on,rect,options);},Height(){W.surface.render(p,c,mode,visible,terrain,{...options,surfaceLayer:'Height'});},
   Water(){W.surface.render(p,c,mode,visible,terrain,{...options,surfaceLayer:'Water'});W.coast?.flow(c,p,rect);},
   Rivers(){W.coast.rivers(c,p,rect,line);},
   Mountains(){for(const s of W.scene.sprites(p,rect,'Mountains'))W.assets.draw(c,s.id,s.x,s.y,s.size,s.variant,.96);},
   Vegetation(){for(const s of W.scene.sprites(p,rect,'Vegetation'))W.assets.draw(c,s.id,s.x,s.y,s.size,s.variant,.96);},
   Roads(){for(const r of p.roads){c.setLineDash(r.kind==='trail'?[3,5]:[]);line(c,r,'#434a354a',r.width+4);line(c,r,'#c0ae80aa',r.width+1);line(c,r,'#ded0a16b',r.width*.35);c.setLineDash([]);}},
   Structures(){for(const o of W.scene.objects(p,rect,'Structures'))object(c,o,o.id===selectedId,mode==='Editor');},
   POI(){for(const o of W.scene.objects(p,rect,'POI'))object(c,o,o.id===selectedId,mode==='Editor');},
   Decorations(){for(const o of W.scene.objects(p,rect,'Decorations'))object(c,o,o.id===selectedId,mode==='Editor');},
   Labels(){if((options.labels??p.styling.labels)!==false)labels(c,p,selectedId,on);}
  };
  for(const l of p.layerOrder)if(on(l))passes[l]?.();
  if((!only||only==='Atmosphere')&&p.styling.atmosphere>0&&visible('Terrain')){const rand=W.random.random(p.seed+'atmosphere');c.save();c.globalAlpha=p.styling.atmosphere/100*.17*(options.atmosphereScale??1);for(let k=0;k<22;k++){const x=rand()*p.width,y=rand()*p.height,e=p.terrain[index(p,x,y)];if(e>.72||e<p.seaLevel)continue;c.save();c.translate(x,y);c.scale(1,.22);const gr=c.createRadialGradient(0,0,0,0,0,100);gr.addColorStop(0,'#e2e3cc');gr.addColorStop(1,'#e2e3cc00');c.fillStyle=gr;c.fillRect(-110,-110,220,220);c.restore();}c.restore();}
