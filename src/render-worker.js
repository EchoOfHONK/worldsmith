...
self.onmessage=async({data:m})=>{try{
 ...
 if(m.type==='exportBand'){await loadFonts();WS.renderer.render(project,canvas,null,{rect:m.rect,resolution:m.resolution,mode:'Atlas',export:true,labels:m.labels,frame:m.frame,textureDetail:m.texture});const image=canvas.transferToImageBitmap();self.postMessage({id:m.id,image,revision},[image]);return;}
