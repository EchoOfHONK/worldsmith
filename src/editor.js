(function(W){
 class Editor {
  ...
  fit(){const w=this.viewport.clientWidth,h=this.viewport.clientHeight;this.zoom=Math.min(w/this.project.width,h/this.project.height)*.97;this.offset={x:(w-this.project.width*this.zoom)/2,y:(h-this.project.height*this.zoom)/2};this.transform();this.changed('view');}
  zoomAt(f,x=this.viewport.clientWidth/2,y=this.viewport.clientHeight/2){const z=Math.round(Math.max(.12,Math.min(W.config.render?.maxZoom||8,this.zoom*f))*1e6)/1e6,r=z/this.zoom;this.offset={x:x-(x-this.offset.x)*r,y:y-(y-this.offset.y)*r};this.zoom=z;this.transform();this.changed('view');}
